//
//  OnboardingCompleteRepository.swift
//  DailyChaCha
//
...
final class OnboardingCompleteRepositoryImpl: OnboardingRepository {
    
    func status() -> Single<Onboarding.Status> {
        .just(.init(isOnboardingCompleted: true))
    }
    
    func progress() -> Single<Onboarding.Progress> {
        .just(.init(progress: .welcome))
    }
    ...
    func dates() -> Single<Onboarding.Dates> {
        .just(.init(goal: .init(goal: "MOCK:몸도 마음도 건강한 삶을 위해", index: 1), exerciseDates: [
            .init(date: 0, time: 0),
            .init(date: 1, time: 0),
            .init(date: 2, time: 0)
        ]))
